(() => {
  ...
  function loadData() {
    try {
      ...
      if (!parsed || !Array.isArray(parsed.subjects)) return sampleData();
  ...
  function handleClick(event) {
    ...
    if (action === "toggle-subtopic") {
      ...
      const topic = findTopic(subjectId, topicId);
      const subtopic = findSubtopic(subjectId, topicId, subtopicId);
      return mutate(subtopic.completed ? "Marked incomplete." : "Marked complete.", () => {
  ...
  function bindEvents() {
    ...
    els.subjectGrid.addEventListener("change", handleClick);
